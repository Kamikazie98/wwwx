...
const initialParams = { ...params, page: initialPage };
      ...
      showLeagueDetails: function(leagueId) {
        ...
        // Force load overview tab initially

        setTimeout(() => {

          
          this.loadLeagueOverview(leagueId);

          
          

          // Double-check tab visibility
      ...
      loadLeagueInfo: function(leagueId) {
        ...
        this.fetchAPI('leagues', { id: leagueId })
          ...
          .then(leagues => {
            ...
            $('#modal-league-logo').attr('src', league.league.logo);

            $('#modal-league-name').text(league.league.name);

            $('#modal-league-country').text(league.country.name);

            $('#modal-league-type').text(league.league.type === 'League' ? 'لیگ' : 'جام');
