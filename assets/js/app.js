...
const initialParams = { ...params, page: initialPage };
              ...
              return `
                <div class="league-item ${iranClass}" data-league-id="${league.league.id}">
                  <div class="league-header">
                    <img src="${league.league.logo}" alt="${league.league.name}" class="league-logo" onerror="this.src=''">
      ...
      showLeagueDetails: function(leagueId) {
        console.log('🎯 showLeagueDetails called with leagueId:', leagueId);
        ...
        console.log('✅ Current league ID set to:', this.currentLeagueId);
        
        // Hide match details sidebar on the left
        $('#match-details-left').hide();
        
        // Show league details section on the left
        $('#league-details-left').show();
        console.log('✅ League details section shown on left sidebar');
        
        // Load league information
        this.loadLeagueInfo(leagueId);
        
        // Bind league sidebar events
        this.bindLeagueSidebarEvents();
        console.log('✅ League sidebar events bound');
        
        // Set default active tab (overview)
        console.log('🔄 About to set active tab to overview...');
        this.setActiveLeagueTab('overview');
        console.log('✅ Default league tab set to overview');
        
        // Don't load overview here - it will be loaded after seasons are populated
      },
      ...
      loadLeagueInfo: function(leagueId) {
        ...
        this.fetchAPI('leagues', { id: leagueId })
          .then(leagues => {
            ...
            $('#left-league-logo').attr('src', league.league.logo);
            $('#left-league-name').text(league.league.name);
            $('#left-league-country').text(league.country.name);
            $('#left-league-type').text(league.league.type === 'League' ? 'لیگ' : 'جام');
            
            // Load seasons first
            this.loadLeagueSeasons(leagueId, league.seasons);
            
            // Don't call loadLeagueOverview here - it will be called by showLeagueDetails
            // after seasons are loaded
      ...
      loadLeagueSeasons: function(leagueId, seasons) {
        console.log('🔄 Loading seasons for league:', leagueId, 'seasons:', seasons);
        
        const selector = $('#left-season-selector');
        selector.empty();
        
        // If no seasons provided, use default seasons
        if (!seasons || !seasons.length) {
          console.log('⚠️ No seasons provided, using default seasons');
          const currentYear = new Date().getFullYear();
          const defaultSeasons = [currentYear, currentYear - 1, currentYear - 2, currentYear - 3];
          ...
          // Set current season (current year)
          selector.val(currentYear);
          console.log('✅ Default seasons loaded, current season:', currentYear);
          
          // Now load the overview with the selected season
          this.loadLeagueOverview(leagueId, currentYear);
          return;
        }
        
        console.log('✅ Seasons array found, length:', seasons.length);
        
        // Sort seasons (newest first)
        const sortedSeasons = seasons.sort((a, b) => {
          const yearA = typeof a === 'object' ? a.year : a;
          const yearB = typeof b === 'object' ? b.year : b;
          return yearB - yearA;
        });
        ...
        sortedSeasons.forEach(season => {
          ...
          const seasonValue = typeof season === 'object' ? season.year : season;
          const seasonText = typeof season === 'object' ? season.year : season;
        ...
        // Set current season (first one)
        if (sortedSeasons.length > 0) {
          const selectedSeason = sortedSeasons[0];
          const seasonValue = typeof selectedSeason === 'object' ? selectedSeason.year : selectedSeason;
          selector.val(seasonValue);
          console.log('✅ Seasons loaded, current season:', seasonValue);
          
          // Now load the overview with the selected season
          this.loadLeagueOverview(leagueId, seasonValue);
        }
      },
      ...
      loadLeagueOverview: function(leagueId, season) {
        const currentSeason = season || $('#left-season-selector').val();
        
        console.log('🔄 Loading league overview for league:', leagueId, 'season:', currentSeason);
        console.log('🔍 League section visible:', $('#league-details-section').is(':visible') ? 'Yes' : 'No');
        console.log('🔍 Overview tab visible:', $('#league-overview').hasClass('active') ? 'Yes' : 'No');
        console.log('🔍 Season selector value:', $('#sidebar-season-selector').val());
        console.log('🔍 Season selector length:', $('#sidebar-season-selector').length);
        console.log('🔍 Season selector options:', $('#sidebar-season-selector option').length);
        
        // Validate season parameter
        if (!currentSeason) {
          console.error('❌ No season parameter provided');
          console.error('❌ Season selector value:', $('#sidebar-season-selector').val());
          console.error('❌ Season selector HTML:', $('#sidebar-season-selector').html());
          $('#sidebar-league-stats').html('<p>خطا: فصل انتخاب نشده است.</p>');
          return;
        }
        
        const seasonInt = parseInt(currentSeason);
        if (isNaN(seasonInt)) {
          console.error('❌ Season parameter is not a valid number:', currentSeason);
          console.error('❌ Season selector value:', $('#sidebar-season-selector').val());
          $('#sidebar-league-stats').html('<p>خطا: فصل نامعتبر است.</p>');
          return;
        }
        
        this.fetchAPI('leagues', { id: leagueId, season: seasonInt })
          .then(leagues => {
            console.log('League overview response:', leagues);
            
            if (!leagues.length) {
              $('#sidebar-league-stats').html('<p>اطلاعات لیگ در دسترس نیست.</p>');
              return;
            }
            ...
            // Update sidebar info fields
            $('#sidebar-tournament').text(league.league?.name || 'نامشخص');
            $('#sidebar-season').text(seasonInfo.year);
            $('#sidebar-stage').text(league.league?.round || 'مرحله عادی');
            $('#sidebar-start').text(seasonInfo.start);
            $('#sidebar-status').text(seasonInfo.current ? 'جاری' : 'پایان یافته');
            
            // Create stats HTML
            const statsHtml = `
              <div class="stats-grid">
                <div class="stat-item">
                  <span class="stat-label">کشور:</span>
                  <span class="stat-value">${league.country?.name || 'نامشخص'}</span>
                </div>
                <div class="stat-item">
                  <span class="stat-label">نوع:</span>
                  <span class="stat-value">${league.league?.type === 'League' ? 'لیگ' : 'جام'}</span>
                </div>
              </div>
            `;
            
            $('#sidebar-league-stats').html(statsHtml);
            console.log('✅ League overview loaded successfully');
          })
          .catch(error => {
            console.error('Error loading league overview:', error);
            $('#sidebar-league-stats').html('<p>خطا در بارگذاری اطلاعات لیگ.</p>');
          });
      },
      ...
      loadLeagueStandings: function(leagueId, season) {
        ...
        console.log('📍 Target element:', $('#sidebar-standings-table').length ? 'Found' : 'NOT FOUND');

        

        // Show loading state

        $('#sidebar-standings-table').html('<div style="text-align: center; padding: 20px;">در حال بارگذاری جدول رده‌بندی...</div>');
        ...
        this.fetchAPI('standings', { league: leagueId, season: currentSeason })
          ...
          .then(standings => {
            ...
            if (!table) {
              ...
              $('#sidebar-standings-table').html('<p>جدول رده‌بندی در دسترس نیست.</p>');
            ...
            $('#sidebar-standings-table').html(standingsHtml);
          ...
          .catch(error => {
            ...
            $('#sidebar-standings-table').html('<p>خطا در بارگذاری جدول رده‌بندی</p>');
        ...
        this.fetchAPI('teams', { league: leagueId, season: currentSeason })
          ...
          .then(teams => {
            ...
            if (!teams.length) {
              ...
              $('#sidebar-teams-grid').html('<p>تیم‌ها در دسترس نیستند.</p>');
            ...
            $('#sidebar-teams-grid').html(teamsHtml);
          ...
          .catch(error => {
            ...
            $('#sidebar-teams-grid').html('<p>خطا در بارگذاری تیم‌ها</p>');
        ...
        this.fetchAPI('fixtures', { league: leagueId, season: currentSeason })
          ...
          .then(fixtures => {
            ...
            if (!fixtures.length) {
              ...
              $('#sidebar-fixtures-list').html('<p>مسابقات در دسترس نیستند.</p>');
            ...
            $('#sidebar-fixtures-list').html(fixturesHtml);
          ...
          .catch(error => {
            ...
            $('#sidebar-fixtures-list').html('<p>خطا در بارگذاری مسابقات</p>');
        ...
        this.fetchAPI('players/topscorers', { league: leagueId, season: currentSeason })
          ...
          .then(scorers => {
            ...
            if (scorers && scorers.length > 0) {
              ...
              $('#sidebar-top-scorers').html(scorersHtml || '<p>اطلاعات آقای گل در دسترس نیست</p>');

            } else {

              $('#sidebar-top-scorers').html('<p>اطلاعات آقای گل در دسترس نیست</p>');
          ...
          .catch(error => {
            ...
            $('#sidebar-top-scorers').html('<p>خطا در بارگذاری آقای گل</p>');
        ...
        this.fetchAPI('players/topassists', { league: leagueId, season: currentSeason })
          ...
          .then(assists => {
            ...
            if (assists && assists.length > 0) {
              ...
              $('#sidebar-top-assists').html(assistsHtml || '<p>اطلاعات پاس‌گل در دسترس نیست</p>');

            } else {

              $('#sidebar-top-assists').html('<p>اطلاعات پاس‌گل در دسترس نیست</p>');
          ...
          .catch(error => {
            ...
            $('#sidebar-top-assists').html('<p>خطا در بارگذاری پاس‌گل</p>');
        ...
        this.fetchAPI('players/topcards', { league: leagueId, season: currentSeason })
          ...
          .then(cards => {
            ...
            if (cards && cards.length > 0) {
              ...
              $('#sidebar-top-cards').html(cardsHtml || '<p>اطلاعات کارت‌ها در دسترس نیست</p>');

            } else {

              $('#sidebar-top-cards').html('<p>اطلاعات کارت‌ها در دسترس نیست</p>');
          ...
          .catch(error => {
            ...
            $('#sidebar-top-cards').html('<p>خطا در بارگذاری کارت‌ها</p>');
      ...
      bindLeagueClickEvents: function() {
        ...
        $(document).on('click', '.league-item', function() {

          const leagueId = $(this).data('league-id');

          console.log('League item clicked:', leagueId);

          app.currentLeagueId = leagueId;

          app.showLeagueDetails(leagueId);
      ...
      setActiveLeagueTab: function(tabId) {
        console.log('🔄 Setting active league tab:', tabId);
        
        // Update active tab button
        $('.league-nav-tabs .tab-list li').removeClass('active');
        $(`.league-nav-tabs .tab-list a[href="#league-${tabId}"]`).parent().addClass('active');
        
        // Show active tab content
        $('.league-details-section .tab-content').removeClass('active');
        $(`#league-${tabId}`).addClass('active');
        
        console.log('✅ League tab switched to:', tabId);
      },

      hideLeagueDetails: function() {
        $('#league-details-section').hide();
        $('#match-details-sidebar').show();
        console.log('✅ League details hidden, match details shown');
      },
      ...
      loadLeagueStandings: function(leagueId, season) {
        ...
        if (!currentSeason || isNaN(parseInt(currentSeason))) {
          console.error('❌ Invalid season parameter:', currentSeason);
          $('#sidebar-standings-table').html('<p>خطا: فصل نامعتبر است.</p>');
          return;
        }
        
        console.log('🔄 Loading league standings for league:', leagueId, 'season:', currentSeason);
        
        this.fetchAPI('standings', { league: leagueId, season: parseInt(currentSeason) })
          .then(standings => {
            ...
            const teams = leagueStandings.league.standings[0] || [];
            
            if (!teams.length) {
              $('#sidebar-standings-table').html('<p>اطلاعات تیم‌ها در دسترس نیست.</p>');
              return;
            }
      ...
      loadLeagueTeams: function(leagueId, season) {
        ...
        if (!currentSeason || isNaN(parseInt(currentSeason))) {
          console.error('❌ Invalid season parameter:', currentSeason);
          $('#sidebar-teams-grid').html('<p>خطا: فصل نامعتبر است.</p>');
          return;
        }
        
        console.log('🔄 Loading league teams for league:', leagueId, 'season:', currentSeason);
        
        this.fetchAPI('teams', { league: leagueId, season: parseInt(currentSeason) })
          .then(teams => {
            console.log('Teams response:', teams);
            
            if (!teams.length) {
              $('#sidebar-teams-grid').html('<p>اطلاعات تیم‌ها در دسترس نیست.</p>');
              return;
            }
                ...
                ${teams.map(team => `
                  <div class="team-card">
                    ...
                    <div class="team-info">
                      ...
                      <p>سال تأسیس: ${team.team.founded || 'نامشخص'}</p>
                    </div>
                  </div>
                `).join('')}
              </div>
            ...
            $('#sidebar-teams-grid').html(teamsHtml);
            console.log('✅ League teams loaded successfully');
          })
          .catch(error => {
      ...
      loadLeagueFixtures: function(leagueId, season) {
        ...
        if (!currentSeason || isNaN(parseInt(currentSeason))) {
          console.error('❌ Invalid season parameter:', currentSeason);
          $('#sidebar-fixtures-list').html('<p>خطا: فصل نامعتبر است.</p>');
          return;
        }
        
        console.log('🔄 Loading league fixtures for league:', leagueId, 'season:', currentSeason);
        
        this.fetchAPI('fixtures', { league: leagueId, season: parseInt(currentSeason) })
          .then(fixtures => {
            console.log('Fixtures response:', fixtures);
            
            if (!fixtures.length) {
              $('#sidebar-fixtures-list').html('<p>مسابقات در دسترس نیست.</p>');
              return;
            }
            
            // Create fixtures list HTML
            const fixturesHtml = `
              <div class="fixtures-list">
                ${fixtures.map(fixture => `
                  <div class="fixture-item">
                    <div class="fixture-date">${this.formatDate(fixture.fixture.date)}</div>
                    <div class="fixture-teams">
                      <div class="team home">
                      ...
                      <div class="fixture-score">
                        <span class="score">${fixture.goals.home || 0} - ${fixture.goals.away || 0}</span>
                        <span class="status">${this.getFixtureStatus(fixture.fixture.status.short)}</span>
                      </div>
                      <div class="team away">
            ...
            $('#sidebar-fixtures-list').html(fixturesHtml);
            console.log('✅ League fixtures loaded successfully');
          })
          .catch(error => {
      ...
      // Load league stats (top scorers and assists)
      loadLeagueStats: function(leagueId, season) {
        const currentSeason = season || $('#sidebar-season-selector').val();
        
        if (!currentSeason || isNaN(parseInt(currentSeason))) {
          console.error('❌ Invalid season parameter:', currentSeason);
          $('#sidebar-top-scorers, #sidebar-top-assists').html('<p>خطا: فصل نامعتبر است.</p>');
          return;
        }
        
        console.log('🔄 Loading league stats for league:', leagueId, 'season:', currentSeason);
        
        // Load top scorers
        this.fetchAPI('players/topscorers', { league: leagueId, season: parseInt(currentSeason) })
          .then(scorers => {
            console.log('Top scorers response:', scorers);
            
            if (!scorers.length) {
              $('#sidebar-top-scorers').html('<p>اطلاعات گل‌زنان در دسترس نیست.</p>');
              return;
            }
            
            const topScorers = scorers.slice(0, 5); // Top 5 scorers
              ...
              <div class="top-scorers-list">
                ${topScorers.map((player, index) => `
                  <div class="player-item">
                    ...
                    <div class="player-info">
                      <span class="player-name">${player.player.name}</span>
                      <span class="team-name">${player.statistics[0]?.team?.name || 'نامشخص'}</span>
                    </div>
                    <div class="stats">
                      <span class="goals">${player.statistics[0]?.goals?.total || 0} گل</span>
                    </div>
                  </div>
                `).join('')}
              </div>
            ...
            $('#sidebar-top-scorers').html(scorersHtml);
            console.log('✅ Top scorers loaded successfully');
          })
          .catch(error => {
            console.error('Error loading top scorers:', error);
            $('#sidebar-top-scorers').html('<p>خطا در بارگذاری گل‌زنان.</p>');
          });
        
        // Load top assists (if available)
        this.fetchAPI('players/topassists', { league: leagueId, season: parseInt(currentSeason) })
          .then(assists => {
            console.log('Top assists response:', assists);
            ...
            const topAssists = assists.slice(0, 5); // Top 5 assists
            
            const assistsHtml = `
              <div class="top-assists-list">
                ${topAssists.map((player, index) => `
                  <div class="player-item">
                    ...
                    <div class="stats">
                      <span class="assists">${player.statistics[0]?.goals?.assists || 0} پاس</span>
                    </div>
                  </div>
                `).join('')}
              </div>
            ...
            $('#sidebar-top-assists').html(assistsHtml);
            console.log('✅ Top assists loaded successfully');
          })
          .catch(error => {
            console.error('Error loading top assists:', error);
            $('#sidebar-top-assists').html('<p>خطا در بارگذاری پاس‌گل.</p>');
      ...
      // Helper function to get fixture status in Persian
      getFixtureStatus: function(status) {
        const statusMap = {
          'NS': 'شروع نشده',
          '1H': 'نیمه اول',
          'HT': 'نیمه',
      ...
      // Bind league sidebar events
      bindLeagueSidebarEvents: function() {
        const app = this;
        
        // Close league details
        $('.close-league-details').on('click', function(e) {
          e.preventDefault();
          app.hideLeagueDetails();
        });
        
        // League tab switching
        $('.league-nav-tabs .tab-list a').on('click', function(e) {
          e.preventDefault();
          const tabId = $(this).attr('href').replace('#league-', '');
          app.setActiveLeagueTab(tabId);
          ...
          if (leagueId && season) {
            app.loadLeagueTabContent(tabId, leagueId, season);
          }
        });
        ...
        $('#sidebar-season-selector').on('change', function() {
          const season = $(this).val();
          const leagueId = app.currentLeagueId;
          const activeTab = $('.league-nav-tabs .tab-list li.active a').attr('href').replace('#league-', '');
          
          if (leagueId && season) {
            app.loadLeagueTabContent(activeTab, leagueId, season);
          }
        });
      },

      // Load league tab content
      loadLeagueTabContent: function(tabId, leagueId, season) {
        console.log('Loading league tab content:', tabId, 'for league:', leagueId, 'season:', season);
        ...
        switch(tabId) {
          case 'overview':
            ...
            break;
          case 'standings':
            this.loadLeagueStandings(leagueId, season);
            break;
          case 'teams':
            this.loadLeagueTeams(leagueId, season);
            break;
          case 'fixtures':
            this.loadLeagueFixtures(leagueId, season);
            break;
          case 'stats':
            this.loadLeagueStats(leagueId, season);
            break;
        }
      },
